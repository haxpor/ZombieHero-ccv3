...
function processIcon() {
    ...
    base_tmp_normalizedFileName="${base_file%.*}-normalized.png"
    base_tmp_path=`dirname $base_path`

    echo "$base_tmp_normalizedFileName"
    echo "$base_tmp_path"

    base_tmp_normalizedFilePath="${base_tmp_path}/${base_tmp_normalizedFileName}"

    echo "$base_tmp_normalizedFilePath"
    ...
    # Remove pngcrush png
    echo "Removing pngcrush png file at $base_path"
    rm "$base_path"
...
echo "Process ipad app icons"

# Workaround to use wildcard * to actually find the file
